...
export class MongoDao<T extends mongoose.Document> {
  ...
  async deleteById(selectionCriteria: Object) {
    let deletedData = await this.model.findByIdAndDelete(selectionCriteria);
    return deletedData;
  }
  async updateByEmail(selectionCriteria: Object, updates: Object) {
    await this.model.find(selectionCriteria, updates);
  }
}
